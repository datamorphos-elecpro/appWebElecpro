...
const labels: Record<string, string> = { name: 'Nombre', client_type: 'Tipo de cliente', contact_name: 'Contacto', phone: 'Teléfono', email: 'Correo', address: 'Dirección', website: 'Sitio web', description: 'Descripción', code: 'Código', unit: 'Unidad', base_unit_price: 'Precio base', category: 'Categoría' };
...
export function CatalogManager({ kind, rows }: { kind: Kind; rows: Row[] }) {
  ...
  const keys = kind === 'clients' ? ['name', 'client_type', 'contact_name', 'phone', 'email', 'address'] : kind === 'suppliers' ? ['name', 'phone', 'email', 'website', 'description'] : ['code', 'description', 'unit', 'base_unit_price', 'category'];
  const filtered = useMemo(() => rows.filter(row => norm(Object.values(row).join(' ')).includes(norm(query))), [rows, query]);
  const required = ['name', 'client_type', 'code', 'description', 'unit', 'base_unit_price'];
