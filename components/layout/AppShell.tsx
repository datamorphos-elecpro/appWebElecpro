...
const links = [
  { href: '/', label: 'Panel general', icon: 'dashboard' },
  ...
  { href: '/finanzas', label: 'Finanzas', icon: 'account_balance_wallet' },
  { href: '/alertas', label: 'Alertas', icon: 'notifications' },
];

const pageTitles: Record<string, { title: string; subtitle: string }> = {
  '/': { title: 'Panel general', subtitle: 'Información consolidada y actualizada al consultar.' },
  ...
  '/finanzas': { title: 'Finanzas', subtitle: 'Consolidado de cartera, gastos y rentabilidad.' },
...
export function AppShell({ children, name, role }: { children: React.ReactNode; name: string; role: string }) {
  ...
  const section = quoteEditor
    ...
    : Object.entries(pageTitles).find(([href]) => href === '/' ? path === '/' : path.startsWith(href))?.[1] ?? { title: 'Elecpro', subtitle: 'Control de proyectos, cotizaciones y rentabilidad.' };
  const roleLabel = role === 'administrator' ? 'Administrador' : 'Gerencia';
    ...
    <div className={styles.layout}>
      <aside className={`${styles.sidebar} ${mobileOpen ? styles.mobileOpen : ''}`} aria-label="Navegación principal">
        <div className={styles.sidebarHead}>
          ...
          <button className={styles.sidebarToggle} type="button" onClick={() => setCollapsed((value) => !value)} aria-label={collapsed ? 'Expandir menú' : 'Contraer menú'} aria-pressed={collapsed}>
            <span className="material-symbols-outlined" aria-hidden="true">{collapsed ? 'left_panel_open' : 'left_panel_close'}</span>
          </button>
        </div>
        <nav className={styles.nav}>
          {links.map((link) => <Link key={link.href} href={link.href} onClick={closeMenu} aria-current={active(link.href) ? 'page' : undefined} title={collapsed ? link.label : undefined}>
            <span className={styles.navIcon}><span className="material-symbols-outlined" aria-hidden="true">{link.icon}</span></span><span className={styles.navLabel}>{link.label}</span>
          </Link>)}
        </nav>
      ...
      <main className={styles.main}>
        <header className={styles.topbar}>
          <div className={styles.topbarStart}>
            <button className={styles.menuToggle} type="button" aria-label="Abrir navegación" aria-expanded={mobileOpen} onClick={() => setMobileOpen(true)}>
              <span className="material-symbols-outlined" aria-hidden="true">menu_open</span>
            </button>
            <div className={styles.topbarTitle}><h1>{section.title}</h1><p>{section.subtitle}</p></div>
          </div>
          <details ref={accountRef} className={styles.account}>
            <summary aria-label="Abrir menú de cuenta">
              <span className={styles.accountText}><strong>{name || 'Usuario Elecpro'}</strong><small>{roleLabel}</small></span>
              <span className={styles.avatar} aria-hidden="true">{initials(name || 'Elecpro')}</span>
            </summary>
            <div className={styles.accountMenu}>
              <strong>{name || 'Usuario Elecpro'}</strong><span>{roleLabel}</span>
              {role === 'administrator' && <Link href="/administracion/usuarios" onClick={closeMenu}><span className="material-symbols-outlined" aria-hidden="true">admin_panel_settings</span>Administración</Link>}
              <form action={signOut}><button type="submit"><span className="material-symbols-outlined" aria-hidden="true">logout</span>Cerrar sesión</button></form>
            </div>
          </details>
        </header>
