...
export default async function QuoteDetail({ params }: { params: Promise<{ quoteId: string }> }) {
  ...
  const [quoteResult, company, { profile }] = await Promise.all([
  ...
  return <QuoteEditor quote={quoteResult.data as any} initialClient={currentClient} company={normalizeCompany(company)} userId={profile.id} />;
}
